...
let solve2 (np : normalized_polynomial) : (float * float option) option =
    ...
    |_-> 
        let first_sol = (((float b.N)*(-1.0)/float b.D) + (rational.Sqrt delta))/(2.0*float a)
        let second_sol = (((float b.N)*(-1.0)/float b.D) - (rational.Sqrt delta))/(2.0*float a)
...
let removeZero (a : polynomial) : polynomial =
    let rec aux a =
        match a with
        Polynomial []->[]
        |Polynomial (x::xs)->
            let (Monomial (q,n))=x
            if q = 0Q then aux (Polynomial xs)
            else x::aux (Polynomial xs)
    in Polynomial (aux a)
